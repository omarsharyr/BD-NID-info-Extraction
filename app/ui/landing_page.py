        ...
        <head>
          ...
          <style>
            ...
            *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;color:var(--ink);background:var(--paper);font-family:"IBM Plex Sans",system-ui,-apple-system,sans-serif;-webkit-font-smoothing:antialiased}button,input{font:inherit}a{color:inherit}h1,h2,h3{font-family:Fraunces,ui-serif,Georgia,serif}.container{width:min(1120px,calc(100% - 40px));margin:auto}.skip{position:fixed;top:10px;left:10px;z-index:20;padding:10px 14px;color:#fff;background:var(--ink);border-radius:10px;transform:translateY(-150%)}.skip:focus{transform:none}:focus-visible{outline:3px solid rgba(0,105,62,.55);outline-offset:3px}
            ...
            .workspace{display:grid;grid-template-columns:.88fr 1.12fr;gap:20px;margin:16px auto 96px}.card{background:var(--panel);border:1px solid var(--line);border-radius:var(--radius-lg);box-shadow:var(--shadow)}.upload-card{padding:28px}.card-head{display:flex;justify-content:space-between;gap:16px;align-items:start;margin-bottom:22px}.card-head h2,.section-head h2{margin:0;font-weight:600;font-size:clamp(1.5rem,2.6vw,2rem);letter-spacing:-.01em}.card-head p,.section-head p{margin:8px 0 0;color:var(--muted);line-height:1.6}.secure{padding:7px 11px;border-radius:999px;color:var(--green-deep);background:rgba(0,105,62,.09);font-size:.74rem;font-weight:700;white-space:nowrap}.drop-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.drop{position:relative;display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:190px;padding:20px;text-align:center;border:1.5px dashed var(--line);border-radius:16px;background:#fbf9f2;transition:.2s ease}.drop.drag{border-color:var(--green);background:rgba(0,105,62,.05)}.drop.has-file{border-style:solid;border-color:#9fcdb2;background:#f2f8f4}.upload-icon{display:grid;place-items:center;width:46px;height:46px;margin:auto auto 12px;border-radius:13px;color:var(--green-deep);background:rgba(0,105,62,.1)}.drop strong{display:block}.drop small{display:block;margin-top:6px;color:var(--muted)}.drop-actions{display:flex;gap:8px;margin-top:14px}.drop-btn{padding:7px 13px;border-radius:999px;border:1px solid var(--line);background:#fff;font-size:.78rem;font-weight:700;color:inherit;cursor:pointer;transition:.15s ease}.drop-btn:hover{border-color:var(--green);color:var(--green-deep)}.submit{width:100%;margin-top:14px}.status{min-height:22px;margin:12px 2px 0;color:var(--muted);font-size:.88rem}.status.error{color:var(--danger)}
              ...
              <form class="card upload-card" id="extract-form">
                ...
                <div class="drop-grid">
                  <div class="drop" data-drop>
                    <input class="drop-input" name="front_image" type="file" accept="image/jpeg,image/png" required hidden>
                    <span class="upload-icon">↑</span><strong>NID front</strong><small>Choose or drop image<br>JPG, JPEG, PNG</small>
                    <div class="drop-actions"><button type="button" class="drop-btn" data-action="camera">📷 Camera</button><button type="button" class="drop-btn" data-action="files">📁 Files</button></div>
                  </div>
                  <div class="drop" data-drop>
                    <input class="drop-input" name="back_image" type="file" accept="image/jpeg,image/png" required hidden>
                    <span class="upload-icon">↑</span><strong>NID back</strong><small>Choose or drop image<br>JPG, JPEG, PNG</small>
                    <div class="drop-actions"><button type="button" class="drop-btn" data-action="camera">📷 Camera</button><button type="button" class="drop-btn" data-action="files">📁 Files</button></div>
                  </div>
                </div>
  ...
  <span class="key">"permanentAddress"</span>: <span class="string">"Cumilla, Bangladesh"</span>
          ...
          <script>
            (()=>{
              ...
              document.querySelectorAll('[data-drop]').forEach(drop=>{const input=drop.querySelector('input'),small=drop.querySelector('small');small.dataset.default=small.innerHTML;const showFile=file=>{drop.classList.add('has-file');small.textContent=file.name};input.addEventListener('change',()=>{if(input.files[0])showFile(input.files[0])});drop.querySelectorAll('[data-action]').forEach(btn=>btn.addEventListener('click',event=>{event.preventDefault();if(btn.dataset.action==='camera')input.setAttribute('capture','environment');else input.removeAttribute('capture');input.value='';input.click()}));['dragenter','dragover'].forEach(name=>drop.addEventListener(name,event=>{event.preventDefault();drop.classList.add('drag')}));drop.addEventListener('dragleave',()=>drop.classList.remove('drag'));drop.addEventListener('drop',event=>{event.preventDefault();drop.classList.remove('drag');if(event.dataTransfer.files[0]){input.files=event.dataTransfer.files;showFile(input.files[0])}}) });
